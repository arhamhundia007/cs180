...
public final class AddController {
    ...
    private void getAddButtonSemantics() {
        ...
        String wholeSaleInput = addView.getWholesalePriceTextField().getText();
        String retailInput = addView.getRetailPriceTextField().getText();
        String numberOfItemsInput = addView.getQuantityTextField().getText();
        ...
        try {
            ...
            wholeSaleAmount = Double.parseDouble(wholeSaleInput);   //input for the wholesale price
        ...
        try {
            ...
            retailAmount = Double.parseDouble(retailInput);   //input for the retail price
        ...
        try {
            ...
            numOfItems = Integer.parseInt(numberOfItemsInput); //input for the num of items
        ...
        Product finalProduct = new Product(sku, name, wholeSaleAmount, retailAmount, numOfItems);

        if (inventoryModel.contains(finalProduct))
            JOptionPane.showMessageDialog(null, "This product could not be added to the" +
                            " inventory!\nA product with the specified SKU already exists!",
                    "Product Inventory", JOptionPane.ERROR_MESSAGE);

        else {
            inventoryModel.add(finalProduct);
